...
const TemplateEditor = () => {
  ...
  // --- NEW: REDUX STATE CONNECTION ---
  const printTemplates = useSelector((state) => state[process.env.REACT_APP_TEMPLATELIBRARY_DATA_KEY]?.printTemplates || []);
  const commTemplates = useSelector((state) => state[process.env.REACT_APP_TEMPLATELIBRARY_DATA_KEY]?.commTemplates || []);
  const isFetching = useSelector((state) => state[process.env.REACT_APP_TEMPLATELIBRARY_DATA_KEY]?.isFetching);
console.log(printTemplates)
  // Combine templates since we support both PRINT and COMMUNICATION
  const templates = useMemo(() => [...printTemplates, ...commTemplates], [printTemplates, commTemplates]);
...
// --- FIX: AWAIT BOTH APIS BEFORE REMOVING SPINNER ---
  useEffect(() => {
    const fetchTemplates = async () => {
      setInitLoading(true);
      try {
          await getTenentTemplateLibrary(dispatch, "PRINT");
          await getTenentTemplateLibrary(dispatch, "COMMUNICATION");
      } catch (error) {
          console.error("Failed to load templates");
      } finally {
          setInitLoading(false); // Guarantees spinner stops!
      }
    };
    fetchTemplates();
  }, [dispatch]);

  // Fetch Variables mapping
  useEffect(() => {
    ...
    fetchConfig();
  }, [selectedType]); // Combined into a single useEffect
      ...
      printDetails: {
        ...
        content: {
          ...
          tableStructure: {
            ...
            summarySettings: { showTax: true, showDiscount: true, showDues: true, wordsAmount: true },
          },
        },
      },
      // Default Communication Details
      commDetails: {
        triggerEvent: "NONE",
        channels: {
          sms: { enabled: false, content: "", templateId: "" },
          email: { enabled: false, subject: "", bodyHtml: "" },
          whatsapp: { enabled: false, templateName: "", headerType: "NONE" },
        },
      },
    };

    try {
        const createdTemplate = await createTemplate(dispatch, newTemplatePayload);
  ...
  const handleSaveClick = async () => {
    try {
      ...
      const apiPayload = {
        ...
        isDefault: values.isDefault !== undefined ? values.isDefault : currentTemplate.isDefault,
      ...
      await updateTemplate(dispatch, editingId, apiPayload);
      message.success("Template saved successfully");
    } catch (error) {
      if (error.errorFields) return; // Ignore Antd validation visual errors
  ...
  return (
    <div style={{ padding: '24px', background: '#f4f6f8', minHeight: '100vh' }}>
      <div style={{ marginBottom: 24 }}>
          <Title level={2} style={{ margin: 0 }}>My Templates</Title>
          <Text type="secondary">Design and manage your custom PDF and communication templates.</Text>
      </div>

      <Card bodyStyle={{ padding: '24px' }} style={{ borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', height: 'calc(100vh - 140px)', overflow: 'hidden' }}>
                ...
                renderItem={(item) => (
                    ...
                    <List.Item.Meta
                        ...
                        <div style={{ fontSize: 11 }}>
                            <Tag color={item.category === 'PRINT' ? 'blue' : 'purple'}>
                                {item.category === 'PRINT' ? item.printDetails?.type : 'DIGITAL'}
                            </Tag>
                ...
                items={[
                    ...
                    children:
                        ...
                        <div style={{ paddingBottom: 40 }}>
                            ...
                            <Card size="small" title="General Settings" style={{ marginBottom: 16 }}>
                            <Row gutter={16}>
                                <Col span={12}>
                            ...
                            <Row gutter={16}>
                                <Col span={16}>
                                <Form.Item name={["commDetails", "triggerEvent"]} label="Automation Trigger" extra="When should this message be sent automatically?">
                                    <Select placeholder="Select Trigger Event">
                                    <Option value="NONE">No Automation (Manual Send Only)</Option>
                                    <Option value="ORDER_CREATED">Order Created</Option>
                                    <Option value="REPORT_READY">Lab Report Ready</Option>
                            ...
                            {/* 2. SMS Channel */}
                            <Card size="small" title="SMS Channel" style={{ marginBottom: 16 }}>
                            <Row gutter={16}>
                                <Col span={6}>
                                <Form.Item name={["commDetails", "channels", "sms", "enabled"]} valuePropName="checked" label="Enable SMS">
                                    <Switch />
                                </Form.Item>
                                </Col>
                                <Col span={18}>
                                <Form.Item name={["commDetails", "channels", "sms", "templateId"]} label="DLT Template ID">
                                    <Input placeholder="1007..." />
                                </Form.Item>
                                </Col>
                                <Col span={24}>
                                <Form.Item name={["commDetails", "channels", "sms", "content"]} label="SMS Content">
                                    <Input.TextArea rows={3} showCount maxLength={160} placeholder="Dear {{patient_name}}, your report is ready..." />
                                </Form.Item>
                                </Col>
                            </Row>
                            </Card>
                            ...
                            {/* 3. Email Channel */}
                            <Card size="small" title="Email Channel" style={{ marginBottom: 16 }}>
                            <Row gutter={16}>
                                <Col span={6}>
                                <Form.Item name={["commDetails", "channels", "email", "enabled"]} valuePropName="checked" label="Enable Email">
                                    <Switch />
                                </Form.Item>
                                </Col>
                                <Col span={18}>
                                <Form.Item name={["commDetails", "channels", "email", "subject"]} label="Email Subject">
                                    <Input placeholder="Lab Report Result..." />
                                </Form.Item>
                                </Col>
                                <Col span={24}>
                                <Form.Item label="Email Body (HTML)" name={["commDetails", "channels", "email", "bodyHtml"]}>
                                    <RichTextEditor placeholder="Draft email body..." availableVariables={config.variables} />
                                </Form.Item>
                                </Col>
                            </Row>
                            </Card>
                            ...
                            {/* 4. WhatsApp Channel */}
                            <Card size="small" title="WhatsApp Channel" style={{ marginBottom: 16 }}>
                            <Row gutter={16}>
                                <Col span={6}>
                                <Form.Item name={["commDetails", "channels", "whatsapp", "enabled"]} valuePropName="checked" label="Enable WhatsApp">
                                    <Switch />
                                </Form.Item>
                                </Col>
                                <Col span={18}>
                                <Form.Item name={["commDetails", "channels", "whatsapp", "templateName"]} label="Meta Template Name" extra="e.g. 'lab_report_v1'">
                                    <Input placeholder="template_name_from_meta" />
                                </Form.Item>
                                </Col>

                                <Col span={12}>
                                <Form.Item name={["commDetails", "channels", "whatsapp", "headerType"]} label="Header Media Type">
                                    <Select>
                                    <Option value="NONE">Text Only (No Media)</Option>
                                    <Option value="DOCUMENT">Document (PDF for Bills/Rx)</Option>
                                    <Option value="IMAGE">Image (Promotions)</Option>
                                    </Select>
                                </Form.Item>
                                </Col>
                                ...
                                <div style={{ color: "#888", fontSize: "12px" }}>
                                    Note: If 'Document' is selected, the system will automatically attach the generated PDF of this template.
                                </div>
                                </Col>
                            </Row>
                            </Card>
                        ...
                        <Card size="small">
                            ...
                            <Form.Item name={["margins", "top"]} noStyle>
                                <InputNumber addonBefore="T" style={{ width: "100%" }} placeholder="0" />
                            </Form.Item>
                            <Form.Item name={["margins", "right"]} noStyle>
                                <InputNumber addonBefore="R" style={{ width: "100%" }} placeholder="0" />
                            </Form.Item>
                            <Form.Item name={["margins", "bottom"]} noStyle>
                                <InputNumber addonBefore="B" style={{ width: "100%" }} placeholder="0" />
                            </Form.Item>
                            <Form.Item name={["margins", "left"]} noStyle>
                                <InputNumber addonBefore="L" style={{ width: "100%" }} placeholder="0" />
                            </Form.Item>
                            </div>
                        </Card>
                        ),
                    ...
                    children: (
                        ...
                        <Card size="small" title="Table Appearance" style={{ marginBottom: 16 }}>
                            <Row gutter={16}>
                            <Col span={8}>
                                <Form.Item name={["content", "tableStructure", "density"]} label="Density">
                                <Select>
                            ...
                            <Col span={8}>
                                <Form.Item name={["content", "tableStructure", "showHead"]} label="Show Header" valuePropName="checked">
                                <Switch />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item name={["content", "tableStructure", "striped"]} label="Zebra Stripes" valuePropName="checked">
                                <Switch />
                                </Form.Item>
                            </Col>
                            </Row>
                        ...
                        <Card size="small" title="Columns Configuration" style={{ marginBottom: 16 }}>
                            ...
                            {(fields, { add, remove, move }) => (
                                ...
                                {fields.map((field, index) => (
                                    ...
                                    <Form.Item {...field} name={[field.name, "width"]} noStyle>
                                        <Input placeholder="%" style={{ width: 80 }} />
                                    </Form.Item>
                                    <Form.Item {...field} name={[field.name, "align"]} noStyle>
                                ...
                                <Button type="dashed" onClick={() => add({ visible: true, width: "auto", align: "left" })} block icon={<PlusOutlined />}>
                                    Add Column
                                </Button>
                                </>
                            )}
