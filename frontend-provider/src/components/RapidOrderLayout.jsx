...
const RapidOrderLayout = ({ onClose, onSwitchToNormal }) => {
  ...
  const { searchResults, orders } = useSelector((state) => state[process.env.REACT_APP_ORDERS_DATA_KEY] || state.order);
...
// --- 1. INITIALIZATION (Runs ONCE on mount) ---
  ...
  const handleServicesChange = (values) => {
    ...
    values.forEach(val => {
      ...
      if (testMatch) newItems.push({ ...testMatch, type: 'Test' });
        ...
        if (pkgMatch) newItems.push({ ...pkgMatch, price: pkgMatch.offerPrice, type: 'Package' });
      }
    });
  ...
  const onFinish = async (values) => {
    ...
    if (res.status === 201) {
      ...
      if (dataToSubmit.paymentMode === "Razorpay" && dataToSubmit.paidAmount > 0) {
        setCreatedOrder({ ...res.data, dueAmountForModal: dataToSubmit.paidAmount });
  ...
  const filterServices = (input, option) => {
      ...
      return text.includes(search) || alias.includes(search);
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: '#f0f2f5', overflow: 'hidden' }}>
        ...
        <div style={{ width: '70vw', display: 'flex', flexDirection: 'column', background: '#fff', borderRight: '1px solid #d9d9d9' }}>
            <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ paymentMode: "Cash", discountAmount: 0, paidAmount: 0 }} style={{height:'100%', display:'flex', flexDirection:'column'}}>
                        ...
                        <Col span={10}>
                            <Input.Group compact>
                                <Select
                                    ...
                                    size="middle"
                                >
                                    {searchResults?.map(p => <Option key={p._id} value={p._id}>{p.firstName} {p.lastName} ({p.mobile})</Option>)}
                                </Select>
                        ...
                        <Col span={14}>
                            {!patientId ? (
                                <Row gutter={8}>
                                    ...
                                    <Col span={6}><InputNumber placeholder="Age" min={1} style={{width:'100%'}} value={patientForm.age} onChange={v => setPatientForm({...patientForm, age: v})} /></Col>
                                    <Col span={8}>
                                        <Select value={patientForm.gender} onChange={v => setPatientForm({...patientForm, gender: v})} style={{width:'100%'}}>
                                    ...
                                    <List
                                        ...
                                        renderItem={item => (
                                            <List.Item
                                                ...
                                                actions={[<DeleteOutlined onClick={() => {
